...
* Kokkos version is 5.1.1, with OpenMP parallel host backend enabled.
...
* In VS Code: In Settings, changing the following settings:
  * `Dev > Containers: Docker Path` from `docker` to `podman`.
  * `Dev > Containers: Docker Compose Path` from `docker-compose` to `podman-compose`.
...
In `devcontainer.json`, one can specify a "postStartCommand". For e.g., it is defined as "cmake -DKokkos_ROOT=/opt/kokkos-install -DCMAKE_EXPORT_COMPILE_COMMANDS=ON -B build" in our case. `-DCMAKE_EXPORT_COMPILE_COMMANDS=ON` ensures that a `build` directory is generated by CMake and that compile commands are exported (helpful for getting IntelliSense to recognize Kokkos header files and API). Otherwise, `CPATH` needs to be updated as follows in `Dockerfile`: `ENV CPATH="/opt/kokkos-install/include:$CPATH"`. See below for some observations with Docker and Podman regarding "postStartCommand".
...
We also report that a user has had no issues with version 5.8.2 of Podman, while working in Fedora 43. Also, that user had to install only the "Dev Containers" extension in VS Code.
