...
TEST(Variant, Intro) {
  ...
  EXPECT_EQ("hello world!", Cexa::Experimental::get<std::string>(v));
  ...
  EXPECT_FALSE(Cexa::Experimental::get_if<int>(&w));
  EXPECT_TRUE(Cexa::Experimental::get_if<std::string>(&w));
  ...
  // single visitation.
  EXPECT_EQ(0, Cexa::Experimental::visit(unary{}, v));
  ...
  // binary visitation.
  EXPECT_EQ(0, Cexa::Experimental::visit(binary{}, v, w));
}
