...
// http://boost.org/LICENSE_1_0.txt) SPDX-FileCopyrightText: Michael Park
...
struct base {
  ...
  KOKKOS_INLINE_FUNCTION static constexpr auto &&get_alt(V &&v) {
    ...
    return recursive_union::get_alt(lib::forward<V>(v).data_,
                                    in_place_index_t<I>{});
#else
    return recursive_union::get_alt(data(lib::forward<V>(v)),
                                    in_place_index_t<I>{});
#endif
  }
};
...
struct base {
  ...
  template <typename Expected, typename Actual> struct visit_return_type_check {
    ...
    KOKKOS_INLINE_FUNCTION static constexpr decltype(auto)
  ...
  template <typename R, typename... ITs> struct dispatcher<false, R, ITs...> {
    ...
    KOKKOS_FORCEINLINE_FUNCTION static constexpr R
    dispatch(F &&, typename ITs::type &&..., Vs &&...) {
      MPARK_BUILTIN_UNREACHABLE;
    }

    template <std::size_t I, typename F, typename... Vs>
    KOKKOS_FORCEINLINE_FUNCTION static constexpr R dispatch_case(F &&,
                                                                 Vs &&...) {
      MPARK_BUILTIN_UNREACHABLE;
    }

    template <std::size_t B, typename F, typename... Vs>
    KOKKOS_FORCEINLINE_FUNCTION static constexpr R dispatch_at(std::size_t,
                                                               F &&, Vs &&...) {
      MPARK_BUILTIN_UNREACHABLE;
    }
  };
...
#undef MPARK_DISPATCH
    ...
    template <std::size_t I, typename F, typename... Vs>
    KOKKOS_FORCEINLINE_FUNCTION static constexpr R dispatch_case(F &&f,
                                                                 Vs &&...vs) {
      using Expected = R;
      using Actual = decltype(lib::invoke(
          lib::forward<F>(f),
...
#undef MPARK_DISPATCH_AT
  ...
  KOKKOS_INLINE_FUNCTION static constexpr const lib::remove_all_extents_t<T> &
  at(const lib::array<T, N> &elems, std::size_t i, Is... is) noexcept {
    return at(elems[i], is...);
  }

  template <typename F, typename... Fs>
  KOKKOS_INLINE_FUNCTION static constexpr lib::array<lib::decay_t<F>,
                                                     sizeof...(Fs) + 1>
  ...
  template <typename F, typename... Vs> struct make_fmatrix_impl {
    ...
    KOKKOS_INLINE_FUNCTION static constexpr dispatch_result_t<F, Vs...>
    dispatch(F &&f, Vs &&...vs) {
...
struct alt {
  ...
  visit_alt(Visitor &&visitor, Vs &&...vs) {
    ...
    return base::dispatcher<
        true, base::dispatch_result_t<Visitor, decltype(as_base(
                                                   lib::forward<Vs>(vs)))...>>::
  ...
  visit_alt_at(std::size_t index, Visitor &&visitor, Vs &&...vs) {
    ...
    return base::dispatcher<
        ...
        template dispatch_at<0>(index, lib::forward<Visitor>(visitor),
                                as_base(lib::forward<Vs>(vs))...);
#elif KOKKOS_COMPILER_MSVC >= 1910
    return base::at(fdiagonal<Visitor &&, decltype(as_base(
...
#define MPARK_VARIANT_RECURSIVE_UNION(destructible_trait, destructor)          \
  ...
  public:                                                                      \
    KOKKOS_INLINE_FUNCTION explicit constexpr recursive_union(                 \
        valueless_t) noexcept                                                  \
        : dummy_{} {}                                                          \
                                                                               \
    template <typename... Args>                                                \
    KOKKOS_INLINE_FUNCTION explicit constexpr recursive_union(                 \
        in_place_index_t<0>, Args &&...args)                                   \
...
public:
  ...
  template <std::size_t I, typename... Args>
  KOKKOS_INLINE_FUNCTION explicit constexpr base(in_place_index_t<I>,
                                                 Args &&...args)
...
protected:
  ...
  friend KOKKOS_INLINE_FUNCTION constexpr const base &as_base(const base &b) {
    return b;
  }
  friend KOKKOS_INLINE_FUNCTION constexpr base &&as_base(base &&b) {
    return lib::move(b);
  }
  friend KOKKOS_INLINE_FUNCTION constexpr const base &&as_base(const base &&b) {
    return lib::move(b);
  ...
  friend KOKKOS_INLINE_FUNCTION constexpr data_t &data(base &b) {
    return b.data_;
  }
  friend KOKKOS_INLINE_FUNCTION constexpr const data_t &data(const base &b) {
    return b.data_;
  }
  friend KOKKOS_INLINE_FUNCTION constexpr data_t &&data(base &&b) {
  ...
  KOKKOS_INLINE_FUNCTION static constexpr std::size_t size() {
    return sizeof...(Ts);
  }
...
#pragma warning(disable : 4100)
  ...
  KOKKOS_INLINE_FUNCTION void operator()(Alt &alt) const noexcept {
    alt.~Alt();
  }
#if defined(KOKKOS_COMPILER_MSVC)
#pragma warning(pop)
...
MPARK_VARIANT_DESTRUCTOR(
    Trait::Available, KOKKOS_FUNCTION ~destructor() { destroy(); },
...
protected:
  ...
  KOKKOS_INLINE_FUNCTION constexpr static T &construct_alt(alt<I, T> &a,
                                                           Args &&...args) {
  ...
  generic_construct(constructor &lhs, Rhs &&rhs) {
    lhs.destroy();
...
MPARK_VARIANT_MOVE_CONSTRUCTOR(
    Trait::Available,
    KOKKOS_FUNCTION constexpr move_constructor(
        move_constructor
            &&that) noexcept(lib::all<std::is_nothrow_move_constructible<Ts>::
                                          value...>::
...
MPARK_VARIANT_COPY_CONSTRUCTOR(
    Trait::Available,
    KOKKOS_FUNCTION constexpr copy_constructor(
        const copy_constructor &that) : copy_constructor(valueless_t{}) {
...
public:
  ...
  template <std::size_t I, typename... Args>
  KOKKOS_INLINE_FUNCTION constexpr /* auto & */ auto emplace(Args &&...args)
...
#pragma warning(pop)
      ...
      struct {
        KOKKOS_FUNCTION void operator()(std::true_type) const {
          this_->emplace<I>(lib::forward<Arg>(arg_));
        }
        KOKKOS_FUNCTION void operator()(std::false_type) const {
  ...
  KOKKOS_INLINE_FUNCTION constexpr void generic_assign(That &&that) {
...
MPARK_VARIANT_COPY_ASSIGNMENT(
    Trait::Available, KOKKOS_FUNCTION constexpr copy_assignment &operator=(
...
public:
  ...
  template <typename Arg,
            ...
            lib::enable_if_t<(std::is_assignable<T &, Arg>::value &&
                             ...
                             int> = 0>
  KOKKOS_INLINE_FUNCTION constexpr variant &operator=(Arg &&arg) noexcept(
  ...
  template <std::size_t I, typename... Args,
            typename T = lib::type_pack_element_t<I, Ts...>,
            lib::enable_if_t<std::is_constructible<T, Args...>::value, int> = 0>
  KOKKOS_INLINE_FUNCTION constexpr T &emplace(Args &&...args) {
  ...
  template <
      ...
      lib::enable_if_t<
          std::is_constructible<T, std::initializer_list<Up> &, Args...>::value,
          int> = 0>
  KOKKOS_INLINE_FUNCTION constexpr T &emplace(std::initializer_list<Up> il,
                                              Args &&...args) {
      ...
      lib::enable_if_t<
          lib::all<Dummy, (lib::dependent_type<std::is_move_constructible<Ts>,
                           ...
                           lib::dependent_type<lib::is_swappable<Ts>,
                                               Dummy>::value)...>::value,
          int> = 0>
  KOKKOS_INLINE_FUNCTION constexpr void swap(variant &that) noexcept(
      lib::all<(std::is_nothrow_move_constructible<Ts>::value &&
...
template <std::size_t I, typename... Ts>
KOKKOS_INLINE_FUNCTION constexpr variant_alternative_t<I, variant<Ts...>> &
get(variant<Ts...> &v) {
...
template <std::size_t I, typename... Ts>
KOKKOS_INLINE_FUNCTION constexpr const variant_alternative_t<I,
                                                             variant<Ts...>> &
...
template <std::size_t I, typename V>
KOKKOS_INLINE_FUNCTION constexpr /* auto * */ auto
...
KOKKOS_INLINE_FUNCTION constexpr bool operator<=(monostate,
                                                 monostate) noexcept {
  return true;
}
...
template <typename Visitor, typename... Vs>
KOKKOS_INLINE_FUNCTION constexpr decltype(auto) visit(Visitor &&visitor,
                                                      Vs &&...vs) {
...
KOKKOS_FUNCTION constexpr bool meets_requirements() noexcept {
  return std::is_copy_constructible<H>::value &&
         std::is_move_constructible<H>::value &&
         lib::is_invocable_r<std::size_t, H, const K &>::value;
}

template <typename K> KOKKOS_FUNCTION constexpr bool is_enabled() noexcept {
  using H = std::hash<K>;
...
private:
  KOKKOS_FUNCTION static std::size_t hash_combine(std::size_t lhs,
                                                  std::size_t rhs) {
...
template <> struct hash<mpark::monostate> {
  ...
  KOKKOS_INLINE_FUNCTION result_type
  operator()(const argument_type &) const noexcept {
