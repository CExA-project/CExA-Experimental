...
#define MPARK_VARIANT_SWITCH_VISIT
      ...
      struct base {
        ...
        struct dispatcher<false, R, ITs...> {
          ...
          KOKKOS_FORCEINLINE_FUNCTION static constexpr R dispatch(
              F &&, typename ITs::type &&..., Vs &&...) {
            MPARK_BUILTIN_UNREACHABLE;
          }

          template <std::size_t I, typename F, typename... Vs>
          KOKKOS_FORCEINLINE_FUNCTION static constexpr R dispatch_case(F &&, Vs &&...) {
            MPARK_BUILTIN_UNREACHABLE;
          }
...
#undef MPARK_DISPATCH
          ...
          template <std::size_t B, typename F, typename V, typename... Vs>
          KOKKOS_FORCEINLINE_FUNCTION static constexpr R dispatch_at(std::size_t index,
                                                             F &&f,
                                                             V &&v,
                                                             Vs &&... vs) {
...
#define MPARK_VARIANT_RECURSIVE_UNION(destructible_trait, destructor)      \
  ...
  union recursive_union<destructible_trait, Index, T, Ts...> {             \
    public:                                                                \
    KOKKOS_INLINE_FUNCTION explicit constexpr recursive_union(valueless_t) \
      noexcept                                                             \
        : dummy_{} {}                                                      \
                                                                           \
    template <typename... Args>                                            \
...
#undef MPARK_VARIANT_DESTRUCTOR
    ...
    class constructor : public destructor<Traits> {
      ...
      KOKKOS_INLINE_FUNCTION constexpr static void generic_construct(constructor &lhs, Rhs &&rhs) {
        lhs.destroy();
...
#define MPARK_VARIANT_COPY_ASSIGNMENT(copy_assignable_trait, definition) \
  ...
  class copy_assignment<traits<Ts...>, copy_assignable_trait>            \
    ...
    MPARK_VARIANT_COPY_ASSIGNMENT(
        Trait::Available,
        KOKKOS_FUNCTION constexpr copy_assignment &operator=(const copy_assignment &that) {
...
#undef MPARK_INHERITING_CTOR
  ...
  class variant {
    ...
    template <
        ...
        lib::enable_if_t<std::is_constructible<T,
                                               ...
                                               Args...>::value,
                         int> = 0>
    KOKKOS_INLINE_FUNCTION constexpr T &emplace(std::initializer_list<Up> il, Args &&... args) {
  ...
  namespace detail {
    ...
    namespace hash {
      ...
      template <typename K>
      KOKKOS_FUNCTION constexpr bool is_enabled() noexcept {
        using H = std::hash<K>;
