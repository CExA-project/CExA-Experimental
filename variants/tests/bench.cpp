...
static void VirtDevice(benchmark::State& state) {
  ...
  Kokkos::parallel_for("initialize", size, KOKKOS_LAMBDA (const int i) {
      ...
      switch (i%3) {
        case 0: 
          new (static_cast<virtDevice::A*>(deviceInstanceMemory + virt_idx)) virtDevice::A();
          break;
        case 1: 
          new (static_cast<virtDevice::B*>(deviceInstanceMemory + virt_idx)) virtDevice::B();
          break;
        case 2: 
          new (static_cast<virtDevice::C*>(deviceInstanceMemory + virt_idx)) virtDevice::C();
          break;
      }
  ...
  for (auto _ : state) {
    Kokkos::parallel_for("loop", 
        ...
        KOKKOS_LAMBDA(int i) {
          size_t virt_idx = deriv_size * i;
          virtDevice::A* a = static_cast<virtDevice::A*>(deviceInstanceMemory + virt_idx);
          res[i] = a->f(i);
        });
    Kokkos::fence();
  }
...
namespace FunctionPtr {
  int fA(int in) { return 4 + in; }
  int fB(int in) { return 6 + in; }
  int fC(int in) { return 2 * fA(in); }
}  // namespace FunctionPtr
...
static void FctPtr(benchmark::State& state) {
  ...
  for (int i = 0; i < size; i+=3) {
    v[i + 0] = &FunctionPtr::fA;
    v[i + 1] = &FunctionPtr::fB;
    v[i + 2] = &FunctionPtr::fC;
  }
...
namespace FunctionPtrDevice {
  KOKKOS_FUNCTION int fA(int in) { return 4 + in; }
  KOKKOS_FUNCTION int fB(int in) { return 6 + in; }
  KOKKOS_FUNCTION int fC(int in) { return 2 * fA(in); }
}  // namespace FunctionPtr
...
static void VariantsNoKokkos(benchmark::State& state) {
  const int size = 1000002;
  Kokkos::View<Kokkos_port::A_Hierarchy*, Kokkos::DefaultHostExecutionSpace> v("view", size);
  auto v_h = Kokkos::create_mirror_view(v);
  for (int i = 0; i < size; i+=3) {
    v_h(i + 0) = Kokkos_port::A{};
    v_h(i + 1) = Kokkos_port::B{};
...
int main(int argc, char** argv) {
  ...
  benchmark::RegisterBenchmark("switch_hostnoKokkos", Switch);
