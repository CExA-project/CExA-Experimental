...
template <class T, std::size_t Size = sizeof(cexa::tuple_size<T>)>
...
template <>
struct cexa::tuple_size<Dummy1> : public std::integral_constant<std::size_t, 0> {};
...
int main(int, char**)
{
  test_complete<cexa::tuple<> >();
  test_complete<cexa::tuple<int&> >();
  test_complete<cexa::tuple<int&&, int&, void*>>();
  ...
  test_incomplete<int>();
  test_incomplete<cexa::tuple<int>&>();
