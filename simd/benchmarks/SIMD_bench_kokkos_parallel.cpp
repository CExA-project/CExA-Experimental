...
template <>
const char* simd_name<Kokkos::Experimental::simd_abi::scalar, float> =
    "scalar float";
template <>
const char* simd_name<Kokkos::Experimental::simd_abi::scalar, double> =
    "scalar double";
...
template <>
const char*
    simd_name<Kokkos::Experimental::simd_abi::avx2_fixed_size<4>, float> =
        "AVX2 4 float";
template <>
const char*
    simd_name<Kokkos::Experimental::simd_abi::avx2_fixed_size<8>, float> =
        "AVX2 8 float";
template <>
const char*
    simd_name<Kokkos::Experimental::simd_abi::avx2_fixed_size<4>, double> =
        "AVX2 4 double";
...
template <>
const char*
    simd_name<Kokkos::Experimental::simd_abi::avx512_fixed_size<8>, float> =
        "AVX512 8 float";
template <>
const char*
    simd_name<Kokkos::Experimental::simd_abi::avx512_fixed_size<16>, float> =
        "AVX512 16 float";
template <>
const char*
    simd_name<Kokkos::Experimental::simd_abi::avx512_fixed_size<8>, double> =
        "AVX512 8 double";
...
static void bench_function(benchmark::State& state) {
  ...
  for (auto _ : state) {
    Kokkos::parallel_for(
        "loop", Kokkos::RangePolicy(0, samples / width),
        KOKKOS_LAMBDA(const std::size_t i) {
          simd_type vec;
          vec.copy_from(&data_test[i * width],
...
BENCHMARK(bench_function<Kokkos::Experimental::simd_abi::scalar, float,
                         Intrinsics::Kokkos>)
    ->Name(std::string("kokkos ") +
           simd_name<Kokkos::Experimental::simd_abi::scalar, float>)
    ->ReportAggregatesOnly(true)
    ->Unit(benchmark::kMillisecond);

BENCHMARK(bench_function<Kokkos::Experimental::simd_abi::scalar, double,
                         Intrinsics::Kokkos>)
    ->Name(std::string("kokkos ") +
           simd_name<Kokkos::Experimental::simd_abi::scalar, double>)
    ->ReportAggregatesOnly(true)
    ->Unit(benchmark::kMillisecond);
