...
void host_check_math_op_accuracy(UnaryOp op) {
  ...
  for (std::size_t i = 0;
       ...
       i += width) {
    vec.copy_from(reinterpret_cast<DataType*>(values),
    ...
    expected_result.copy_from(expected,
                              Kokkos::Experimental::simd_flag_default);
...
inline void host_check_math_ops_accuracy_all_types(
    Kokkos::Experimental::Impl::data_types<DataTypes...>) {
  (host_check_all_math_ops_accuracy<Abi, DataTypes>(), ...);
}

template <typename... Abis>
inline void host_check_math_ops_accuracy_all_abis(
...
TEST(simd, host_math_ops_accuracy) {
  host_check_math_ops_accuracy_all_abis(
