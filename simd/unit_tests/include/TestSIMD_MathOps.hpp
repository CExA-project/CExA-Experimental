...
template <bool check_relative_error, class Abi, class Loader, class BinaryOp,
          class T>
...
template <bool check_relative_error, class Abi, class Loader, class UnaryOp,
          class T>
...
inline void host_check_math_op_all_loaders(Op op, std::size_t n,
                                           T const*... args) {
  host_check_math_op_one_loader<check_relative_error, Abi,
                                load_element_aligned>(op, n, args...);
  host_check_math_op_one_loader<check_relative_error, Abi, load_masked>(
      op, n, args...);
  host_check_math_op_one_loader<check_relative_error, Abi, load_as_scalars>(
      op, n, args...);
  host_check_math_op_one_loader<check_relative_error, Abi, load_vector_aligned>(
      op, n, args...);
}
...
inline void host_check_all_math_ops(const DataType (&first_args)[n],
  ...
  host_check_math_op_all_loaders<check_relative_error, Abi>(exp_op(), n,
                                                            first_args);
}
...
inline void host_check_math_ops() {
  if constexpr (is_type_v<Kokkos::Experimental::basic_simd<DataType, Abi>>) {
    ...
    if constexpr (!std::is_integral_v<DataType>) {
      ...
      host_check_all_math_ops<check_relative_error, Abi>(first_args,
                                                         second_args);
    } else {
...
inline void host_check_math_op_special_values_all_loaders(Op op) {
  op.template check_special_values<check_relative_error, Abi,
                                   load_element_aligned, DataType>();
  op.template check_special_values<check_relative_error, Abi, load_masked,
                                   DataType>();
  op.template check_special_values<check_relative_error, Abi, load_as_scalars,
                                   DataType>();
  op.template check_special_values<check_relative_error, Abi,
                                   load_vector_aligned, DataType>();
}

template <bool check_relative_error, typename Abi, typename DataType>
inline void host_check_all_math_ops_special_values() {
  host_check_math_op_special_values_all_loaders<check_relative_error, Abi,
                                                DataType>(exp_op());
}
template <bool check_relative_error, typename Abi, typename DataType>
inline void host_check_math_ops_special_values() {
  if constexpr (is_type_v<Kokkos::Experimental::basic_simd<DataType, Abi>>) {
    ...
    host_check_abi_size<Abi, DataType>();
    host_check_all_math_ops_special_values<check_relative_error, Abi,
                                           DataType>();
  }
}
...
inline void host_check_math_ops_special_values_all_abis(
    Kokkos::Experimental::Impl::abi_set<Abis...>) {
  using DataTypes = Kokkos::Experimental::Impl::data_type_set;
  (host_check_math_ops_special_values_all_types<check_relative_error, Abis>(
       DataTypes()),
   ...);
}

TEST(simd, host_math_ops) {
  host_check_math_ops_all_abis<false>(
      Kokkos::Experimental::Impl::host_abi_set());
}

TEST(simd, host_math_ops_special_values) {
  host_check_math_ops_special_values_all_abis<false>(
      Kokkos::Experimental::Impl::host_abi_set());
}

TEST(simd, host_math_ops_rel_error) {
  host_check_math_ops_all_abis<true>(
      Kokkos::Experimental::Impl::host_abi_set());
}

TEST(simd, host_math_ops_special_values_rel_error) {
  host_check_math_ops_special_values_all_abis<true>(
      Kokkos::Experimental::Impl::host_abi_set());
}
