...
// Convert a packed double to packed i64 (the intrinsic for this is only
// available for avx512). We need to store because msvc doesn't support proper
// indexing on simd vectors.
inline __m256i cvtpd_epi64(__m256d x) {
  alignas(__m256d) double buf[4];
  _mm256_store_pd(buf, x);
  return _mm256_setr_epi64x(
      static_cast<std::int64_t>(buf[0]), static_cast<std::int64_t>(buf[1]),
      static_cast<std::int64_t>(buf[2]), static_cast<std::int64_t>(buf[3]));
}
...
inline __m256d cvtepi64_pd(__m256i x) {
  ...
  return _mm256_setr_pd(
      ...
      static_cast<double>(buf[2]), static_cast<double>(buf[3]));
}

// Shifts a vector of 4xi64 by `count` (`count` < 32).
inline __m256i srai_epi64(__m256i x, int count) {
  __m256i sign_mask =
      _mm256_and_si256(x, _mm256_set1_epi64x(0x8000000000000000));
...
// Ping-Tak Peter Tang. 1989. Table-driven implementation of the exponential
// function in IEEE floating-point arithmetic. ACM Trans. Math. Softw. 15, 2
// (June 1989), 144–157. https://doi.org/10.1145/63522.214389
inline __m256d exp4d(__m256d x) {
  using namespace constants::double_precision;

  const __m256d N =
      _mm256_round_pd(_mm256_mul_pd(x, _mm256_set1_pd(INV_L)),
  ...
  const __m256d R2 =
      _mm256_fnmadd_pd(N, _mm256_set1_pd(L2), _mm256_set1_pd(0.0));
  ...
  two_M         = _mm256_blendv_pd(
      two_M, _mm256_set1_pd(std::numeric_limits<double>::infinity()),
      _mm256_castsi256_pd(large_exponent_mask));

  const __m256d p2 =
      _mm256_add_pd(S_lead_vec, _mm256_fmadd_pd(S, P, S_trail_vec));
  ...
  res = _mm256_blendv_pd(res, _mm256_set1_pd(0.0),
                         _mm256_or_pd(minus_inf_mask, x_neg_over_t1_mask));
  res = _mm256_blendv_pd(res, _mm256_add_pd(_mm256_set1_pd(1.0), x),
                         abs_x_lt_t2_mask);
...
inline __m256 exp8f(__m256 x) {
  ...
  const __m256 N =
      _mm256_round_ps(_mm256_mul_ps(x, _mm256_set1_ps(INV_L)),
                      _MM_FROUND_TO_NEAREST_INT | _MM_FROUND_NO_EXC);
  ...
  const __m256 R2 =
      _mm256_fnmadd_ps(N, _mm256_set1_ps(L2), _mm256_set1_ps(0.0f));
  ...
  two_M        = _mm256_blendv_ps(
      two_M, _mm256_set1_ps(std::numeric_limits<float>::infinity()),
      _mm256_castsi256_ps(large_exponent_mask));

  const __m256 p2 =
      _mm256_add_ps(S_lead_vec, _mm256_fmadd_ps(S, P, S_trail_vec));
  ...
  res = _mm256_blendv_ps(res, _mm256_set1_ps(0.0f),
                         _mm256_or_ps(minus_inf_mask, x_neg_over_t1_mask));
  res = _mm256_blendv_ps(res, _mm256_add_ps(_mm256_set1_ps(1.0f), x),
                         abs_x_lt_t2_mask);
...
inline __m128 exp4f(__m128 x) {
  ...
  const __m128 N   = _mm_round_ps(_mm_mul_ps(x, _mm_set1_ps(INV_L)),
                                  _MM_FROUND_TO_NEAREST_INT | _MM_FROUND_NO_EXC);
  ...
  const __m128 R1_ge_512 =
      _mm_fnmadd_ps(N2, vec_L1, _mm_fnmadd_ps(N1, vec_L1, x));
  ...
  const __m128i large_exponent_mask =
      _mm_cmpgt_epi32(M, _mm_set1_epi32(BIAS + 1));
  __m128 two_M = _mm_castsi128_ps(_mm_sllv_epi32(shifted_value, shift));
  two_M =
      _mm_blendv_ps(two_M, _mm_set1_ps(std::numeric_limits<float>::infinity()),
  ...
  __m128 res =
      _mm_mul_ps(_mm_mul_ps(two_M, p2),
  ...
  const __m128 x_over_t1_mask =
      _mm_cmp_ps(x, _mm_set1_ps(THRESHOLD_1), _CMP_GT_OQ);
  ...
  res = _mm_blendv_ps(res, _mm_set1_ps(0.0f),
