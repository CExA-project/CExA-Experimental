...
inline __m512d exp8d(__m512d x) {
  ...
  const __m512d N =
      _mm512_roundscale_pd(_mm512_mul_pd(x, _mm512_set1_pd(INV_L)),
  ...
  const __m512d R2 =
      _mm512_fnmadd_pd(N, _mm512_set1_pd(L2), _mm512_set1_pd(0.0));
  ...
  const __mmask8 mul_mask =
      _mm512_cmpeq_epi64_mask(M, _mm512_set1_epi64(BIAS + 1));
  shifted_value =
      _mm512_mask_sub_epi64(shifted_value, mul_mask, shifted_value, one);
  ...
  two_M         = _mm512_mask_blend_pd(
      large_exponent_mask, two_M,
      _mm512_set1_pd(std::numeric_limits<double>::infinity()));

  const __m512d p2 =
      _mm512_add_pd(S_lead_vec, _mm512_fmadd_pd(S, P, S_trail_vec));
  ...
  res = _mm512_mask_blend_pd(minus_inf_mask | x_neg_over_t1_mask, res,
                             _mm512_set1_pd(0.0));
  res = _mm512_mask_blend_pd(abs_x_lt_t2_mask, res,
...
inline __m512 exp16f(__m512 x) {
  ...
  const __m512 N =
      _mm512_roundscale_ps(_mm512_mul_ps(x, _mm512_set1_ps(INV_L)),
                           _MM_FROUND_TO_NEAREST_INT | _MM_FROUND_NO_EXC);
  const __m512i Ni  = _mm512_cvtps_epi32(N);
  const __m512i N2i = _mm512_and_si512(
      Ni, _mm512_set1_epi32(32 - 1));  // N2i = Ni % 32, N2i >= 0
  const __m512i N1i = Ni - N2i;
  ...
  const __m512 R2 =
      _mm512_fnmadd_ps(N, _mm512_set1_ps(L2), _mm512_set1_ps(0.0f));
  ...
  const __mmask16 mul_mask =
      _mm512_cmpeq_epi32_mask(M, _mm512_set1_epi32(BIAS + 1));
  shifted_value =
      _mm512_mask_sub_epi32(shifted_value, mul_mask, shifted_value, one);
  ...
  const __m512 p2 =
      _mm512_add_ps(S_lead_vec, _mm512_fmadd_ps(S, P, S_trail_vec));
  ...
  res = _mm512_mask_blend_ps(minus_inf_mask | x_neg_over_t1_mask, res,
                             _mm512_set1_ps(0.0f));
  res = _mm512_mask_blend_ps(abs_x_lt_t2_mask, res,
...
inline __m256 exp8f(__m256 x) {
  ...
  const __m256 N =
      _mm256_roundscale_ps(_mm256_mul_ps(x, _mm256_set1_ps(INV_L)),
                           _MM_FROUND_TO_NEAREST_INT | _MM_FROUND_NO_EXC);
  const __m256i Ni  = _mm256_cvtps_epi32(N);
  const __m256i N2i = _mm256_and_si256(
      Ni, _mm256_set1_epi32(32 - 1));  // N2i = Ni % 32, N2i >= 0
  ...
  const __mmask8 n_ge_512 =
      _mm256_cmp_ps_mask(abs_N, _mm256_set1_ps(512), _CMP_GE_OQ);
  ...
  const __mmask8 mul_mask =
      _mm256_cmpeq_epi32_mask(M, _mm256_set1_epi32(BIAS + 1));
  shifted_value =
      _mm256_mask_sub_epi32(shifted_value, mul_mask, shifted_value, one);
  const __mmask8 large_exponent_mask =
  ...
  res = _mm256_mask_blend_ps(minus_inf_mask | x_neg_over_t1_mask, res,
                             _mm256_set1_ps(0.0f));
  res = _mm256_mask_blend_ps(abs_x_lt_t2_mask, res,
